...
/// Unshare with raw u32 flags
pub fn unshareRaw(flags: u32) SyscallError!void {
    const result = linux.syscall1(.unshare, flags);
    const errno = linux.E.init(result);
    return switch (errno) {
        .SUCCESS => {},
        .PERM => error.PermissionDenied,
...
/// prctl wrapper
pub fn prctl(option: u32, arg2: usize, arg3: usize, arg4: usize, arg5: usize) SyscallError!void {
...
pub fn makedev(major: u32, minor: u32) u64 {
    return (@as(u64, major) << 8) | @as(u64, minor);
}

/// mknod wrapper (uses mknodat with AT_FDCWD for aarch64 compatibility)
pub fn mknod(path: [*:0]const u8, mode: u32, dev: u64) SyscallError!void {
    const AT_FDCWD: usize = @bitCast(@as(isize, -100));
    const result = linux.syscall4(.mknodat, AT_FDCWD, @intFromPtr(path), mode, dev);
    return switch (linux.E.init(result)) {
        .SUCCESS => {},
        .PERM, .ACCES => error.PermissionDenied,
        .EXIST => {}, // already exists, ok
        else => error.Unexpected,
    };
}
